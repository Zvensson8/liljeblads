...
serve(async (req) => {
  ...
  try {
    ...
    if (!isLlmConfigured()) {
      throw new Error('GOOGLE_AI_API_KEY (or XAI_API_KEY) is not configured');
    }
...
- Energieffektiviseringsmöjligheter`;
    ...
    let aiData: Awaited<ReturnType<typeof chatCompletion>>;
    try {
    ...
    if (aiData instanceof Response) {
      throw new Error('Unexpected stream from protocol analysis');
    }
