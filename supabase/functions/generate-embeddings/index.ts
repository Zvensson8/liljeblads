...
import { assertCronOrAdminAuthorized, cronCorsHeaders } from "../_shared/cronAuth.ts";
...
/** Call parse-document with service role so cron/worker can extract PDF text. */
async function parsePdfWithServiceRole(url: string, maxPages = 15): Promise<string> {
  try {
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
    ...
    if (!parseResponse.ok) {
      console.error('parse-document status', parseResponse.status, await parseResponse.text());
      return '';
    }
    const parsed = await parseResponse.json();
    return (parsed.text as string) || '';
  } catch (e) {
    console.error('parsePdfWithServiceRole error:', e);
    return '';
  }
}

async function fetchTextFileWithServiceRole(url: string): Promise<string> {
  const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
  const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
  ...
  if (publicMatch) {
    const { data, error } = await supabase.storage.from(publicMatch[1]).download(publicMatch[2]);
    if (error || !data) throw error || new Error('download failed');
    return (await data.text()).substring(0, 12000);
  }
  const resp = await fetch(url);
  if (!resp.ok) throw new Error(`fetch ${resp.status}`);
  return (await resp.text()).substring(0, 12000);
}
  ...
  switch (sourceTable) {
    ...
    case 'property_documents': {
      const { data, error } = await supabase
        ...
        .select(`
          name, file_url, mime_type, file_size, version, is_latest,
          property:properties!inner(id, name, address, organization_id)
        `)
        .eq('id', sourceId)
        .single();

      if (error || !data) return null;
      if (data.is_latest === false) return null;
      ...
      if (isPdf) {
        pdfContent = await parsePdfWithServiceRole(data.file_url);
      } else if (
        data.mime_type?.startsWith('text/') ||
        data.name?.toLowerCase().match(/\.(txt|md|csv)$/)
      ) {
        try {
          pdfContent = await fetchTextFileWithServiceRole(data.file_url);
        } catch (e) {
          console.error('property_documents text fetch failed:', e);
        }
      }
      ...
      const parts = [
        `[FASTIGHETSDOKUMENT] Fil: ${data.name}`,
        prop?.name ? `Fastighet: ${prop.name}` : '',
        prop?.address ? `Adress: ${prop.address}` : '',
        prop?.id ? `property_id: ${prop.id}` : '',
        data.version ? `Version: ${data.version}` : '',
        data.mime_type ? `Typ: ${data.mime_type}` : '',
      ].filter(Boolean);
      ...
      if (pdfContent) {
        ...
        parts.push(
          'OBS: Textinnehåll kunde inte extraheras (skannad PDF eller tom fil).',
        );
      }

      return {
        content: parts.join('. '),
        organizationId: prop?.organization_id || null,
      };
    }
    ...
    case 'maintenance_history_documents': {
      ...
      if (data.file_url && (data.file_name?.toLowerCase().endsWith('.pdf') || data.mime_type === 'application/pdf')) {
        ...
        pdfContent = await parsePdfWithServiceRole(data.file_url, 10);
        if (pdfContent) {
