...
const WorkOrders = () => {
  ...
  const archived = workOrders?.filter((wo) => wo.status === "archived") || [];
  const orderedTotal = ordered.reduce((sum, wo) => sum + (Number(wo.price) || 0), 0);
  const completedTotal = completed.reduce((sum, wo) => sum + (Number(wo.price) || 0), 0);
  const archivedTotal = archived.reduce((sum, wo) => sum + (Number(wo.price) || 0), 0);
            ...
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              {viewMode === "kanban" ? (
                ...
                <div className="space-y-4">
                  {showArchived ? (
                    <>
                      {renderOrdersTable(completed, "Slutförda", "✅", completedTotal)}
                      {renderOrdersTable(archived, "Arkiverade", "📦", archivedTotal)}
                    </>
                  ) : (
                    <>
