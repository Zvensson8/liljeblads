...
export default function ComponentDetail() {
  ...
  const serviceCount = uniqueServiceDates.length;
  const averageMaintenanceCost = serviceCount > 0 ? totalMaintenanceCost / serviceCount : 0;
    ...
    <SidebarProvider>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card
                  ...
                  onClick={() => maintenanceHistory.length > 0 && setServiceListOpen(true)}
                >
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                ...
                <Card
                  className="cursor-pointer hover:bg-muted/50 transition-colors"
                  onClick={() => serviceCount > 0 && setServiceListOpen(true)}
                >
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  ...
                  <CardContent>
                    ...
                    {serviceCount > 0 && (
                      <p className="text-xs text-muted-foreground mt-1">
                        {maintenanceHistory.length} {maintenanceHistory.length === 1 ? 'åtgärd' : 'åtgärder'} totalt
                      </p>
                    )}
                  </CardContent>
                </Card>
              ...
              <Tabs defaultValue="info" className="w-full">
                <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 lg:grid-cols-7">
                  ...
                  <TabsTrigger value="service">
                    Service{maintenanceHistory.length > 0 && ` (${maintenanceHistory.length})`}
                  </TabsTrigger>
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <CardTitle>Registrerad service</CardTitle>
                        <QuickServiceButton
                          componentId={component.id}
                          componentName={component.name}
                          onSuccess={fetchComponentData}
                    ...
                    <CardContent>
                      <ServiceListTab records={maintenanceHistory} onChanged={fetchComponentData} />
                    </CardContent>
                  </Card>
                </TabsContent>
      ...
      <Dialog open={serviceListOpen} onOpenChange={setServiceListOpen}>
        <DialogContent className="max-w-xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Registrerad service ({maintenanceHistory.length})</DialogTitle>
          </DialogHeader>
          <ServiceListTab records={maintenanceHistory} onChanged={fetchComponentData} compact />
        </DialogContent>
      </Dialog>
    </SidebarProvider>
  );
