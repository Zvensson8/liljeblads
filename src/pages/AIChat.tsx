...
const suggestedQuestions = [
  ...
  "Skicka fakturaadressen för Nolhaga 2 till mig",
  "Lista aktiva arbetsordrar",
  "Skapa en arbetsorder för service av värmepump",
  "Ändra status på senaste arbetsordern till ordered",
];
...
export default function AIChat({ embedded = false }: { embedded?: boolean }) {
  ...
  /** Non-streaming chat so Jarvis tools (status, WO, docs, utkast) can run. */
  const callJarvisChat = async (
    ...
    if (!response.ok) {
      const err = new Error(payload.error || `HTTP ${response.status}`) as Error & { status?: number };
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      if (result.suggestedActions && (result.suggestedActions as unknown[]).length > 0) {
        toast.success(
          `${(result.suggestedActions as unknown[]).length} förslag sparat — godkänn i AI-inkorgen`,
        );
