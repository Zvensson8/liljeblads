...
export const WorkOrderKanban = ({ workOrders, onEdit, onDelete, onViewDetails, onRefetch }: WorkOrderKanbanProps) => {
  ...
  const handleDragStart = (order: WorkOrder) => {
  ...
  const handleDrop = async (newStatus: string) => {
    ...
    const { error } = await supabase
      .from("work_orders")
      .update({ status: newStatus as "not_started" | "awaiting_quote" | "ordered" | "completed" | "archived" })
  ...
  const getOrdersForColumn = (columnId: string) => {
    return workOrders.filter((wo) => wo.status === columnId);
  };

  const calculateTotal = (orders: WorkOrder[]) => {
    return orders.reduce((sum, wo) => sum + (Number(wo.price) || 0), 0);
  };
