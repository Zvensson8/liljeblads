...
export function PropertyTodos({ propertyId, compact = false }: PropertyTodosProps) {
  ...
  const undoableDelete = useUndoableMutation<{ id: string }>({
    label: "Uppgift borttagen",
    action: async ({ id }) => {
      await deleteTodo.mutateAsync(id);
    },
    optimistic: ({ id }) => {
      const key = queryKeys.todos.list({ propertyId });
      const prev = queryClient.getQueryData<Todo[]>(key);
      queryClient.setQueryData<Todo[]>(key, (old) =>
        (old ?? []).filter((t) => t.id !== id && t.parent_todo_id !== id),
      );
      return () => queryClient.setQueryData(key, prev);
    },
    onError: () => toast.error("Kunde inte ta bort uppgiften"),
  });
