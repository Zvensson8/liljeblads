...
export interface ComponentRiskResult {
  ...
  /** Enriched by batch list hooks (optional) */
  name?: string;
  type?: string | null;
  propertyId?: string | null;
  propertyName?: string | null;
...
export type ComponentRiskListItem = ComponentRiskResult;
...
export function riskLevelMeetsMin(level: RiskLevel, min?: RiskLevel): boolean {
  if (!min) return true;
  return RISK_LEVEL_RANK[level] >= RISK_LEVEL_RANK[min];
}

export function confidenceMeetsMin(c: Confidence, min?: Confidence): boolean {
  if (!min) return true;
...
/** Batch helper – compute risk for many components (highest risk first) */
  ...
  return inputs
    .map((input) => {
      ...
      return {
        ...
        name: input.name,
        type: input.type,
        propertyId: (input as ComponentRiskInput & { propertyId?: string }).propertyId,
        propertyName: (input as ComponentRiskInput & { propertyName?: string }).propertyName,
      };
    })
    .sort((a, b) => {
...
export function filterRiskResults(
  results: ComponentRiskResult[],
  opts: { minLevel?: RiskLevel; minConfidence?: Confidence; limit?: number } = {},
): ComponentRiskResult[] {
  let out = results.filter(
    (r) =>
      riskLevelMeetsMin(r.riskLevel, opts.minLevel) &&
      confidenceMeetsMin(r.confidence, opts.minConfidence),
  );
